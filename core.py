# core.py
# Art Mash — Flat-file core module (no subfolders)
# Contains DB, fetch/cache, parsers, ingest, rating, leaderboards, rendering, and queue helpers.
...
def next_from_queue(st_session_state) -> Optional[str]:
    q = st_session_state.get("queue_urls", [])
    idx = st_session_state.get("queue_idx", 0)
    if not q:
        return None
    url = q[idx % len(q)]
    st_session_state["queue_idx"] = (idx + 1) % len(q)
    return url
